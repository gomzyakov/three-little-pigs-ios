...
/**
 Создаем надпись с текстом сцены.
 @note Учитывая то, что SKLabelNode не позволяет нам создавать многострочный
 текст, создаем некоторый корневой узел к которому и цепляем наши надписи.
 */
- (void)setupTextLabel
{
    CGFloat TLPFontSize = 32.0;

    NSString *text1 = NSLocalizedStringWithDefaultValue(@"SCENE_010_TEXT", @"Localizable", [NSBundle mainBundle], @"Жили были три поросенка.", @"Первая строка текста первой сцены");
    NSString *text2 = NSLocalizedStringWithDefaultValue(@"SCENE_010_TEXT", @"Localizable", [NSBundle mainBundle], @"Они жили на ферме с мамой и папой", @"Вторая строка текста первой сцены");

    SKNode *nerdText = [SKNode node];

    SKLabelNode *label1 = [SKLabelNode labelNodeWithFontNamed:@"PTSerif-Regular"];
    label1.fontSize                = TLPFontSize;
    label1.fontColor               = [UIColor purpleColor];
    label1.text                    = text1;
    label1.horizontalAlignmentMode = SKLabelHorizontalAlignmentModeLeft;

    SKLabelNode *label2 = [SKLabelNode labelNodeWithFontNamed:@"PTSerif-Regular"];
    label2.fontSize                = TLPFontSize;
    label2.fontColor               = [UIColor purpleColor];
    label2.text                    = text2;
    label2.horizontalAlignmentMode = SKLabelHorizontalAlignmentModeLeft;

    label2.position = CGPointMake(label2.position.x, label2.position.y - TLPFontSize*1.25);
    [nerdText addChild:label1];
    [nerdText addChild:label2];
    nerdText.position = CGPointMake(100, 630.0);
